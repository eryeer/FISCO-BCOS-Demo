...
library LibString {
    ...
    function split(string _self, string _delim, string[] storage _array) internal {
        ...
        while (true) {
            ...
            string memory elem = new string(ptr-pos+1);
            uint elemptr;
            assembly {
                elemptr := add(elem, 0x20)
            }
            memcpy(elemptr, selfptr+pos, ptr-pos+1);
    ...
    function toHex(string _self) internal returns (bytes _ret) {
    ...
    function getArrayValueByKey(string _self, string _key, string[] storage _array) internal {
        ...
        string memory _delim1 = "{";
        //string memory _delim2 = "}";
        vals.split("}", _array);
...
/*
    function splitArray(string _self, string[] storage _array) internal {

        int start = 0;
        int end = int(bytes(_self).length);
        int tmpPos = start;
        while(tmpPos < end){
            int tmpStart = _self.indexOf("{", tmpPos);
            if(tmpStart == -1 || tmpStart >= end){
                break;
            }
            int tmpEnd = _self.indexOf("}", tmpPos);
            if(tmpEnd == -1){
                break;
            }
            string memory tmpStr = _self.substr(uint(tmpStart), uint(tmpEnd-tmpStart)).trim(" \t\r\n");
